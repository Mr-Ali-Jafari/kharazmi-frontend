    ...
    if (inputValue.trim()) {
      ...
      try {
        ...
        if (response.ok && data.answer) {
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { text: "پاسخی از سرور دریافت نشد.", sender: "bot" },
          ]);
        }
      } catch (error) {
        setMessages((prevMessages) => [
          ...prevMessages,
          { text: "خطا در ارتباط با سرور.", sender: "bot" },
        ]);
      }
      setInputValue("");
      setLoading(false);
  ...
  return (
    <div className={`flex-1 transition-all duration-300 ${isSidebarExpanded ? "ml-64" : "ml-16"} h-screen flex flex-col`}>
        ...
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            <div key={index} className="flex items-start mb-3 animate-slide-in">
              ...
              />
              <div className={`bg-gradient-to-r ${msg.sender === "user" ? "from-gray-800 to-gray-700" : "from-purple-800 to-blue-700"} text-white p-3 shadow-md`}>
      ...
      <div className="p-4 bg-gray-800 border-t border-gray-700 shadow-inner flex items-center">
        <button className="mr-2 text-yellow-400 hover:text-yellow-300">
          <FaSmile size={24} />
        </button>
        <input
          type="text"
          ...
          className="flex-1 border border-gray-600 p-3 bg-gray-700 text-white placeholder-gray-400"
          placeholder="سوال خود را بپرسید..."
        />
        <button
          onClick={handleSendMessage}
          className="bg-blue-600 text-white px-4 py-3 ml-3 shadow-md"
          disabled={loading}
        >
